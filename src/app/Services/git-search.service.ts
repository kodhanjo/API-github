...
export class GitSearchService {
  ...
  getMyInfo(): Promise<any> {
    return new Promise((resolve, reject) => {
      const apiUrl = `https://api.github.com/users/kodhanjo?access_token=${environment.apiUrl}`;
      this.http
        .get<any>(apiUrl)
        .toPromise()
        .then(res => {
  ...
  getMyRepos(): Promise<any> {
    return new Promise((resolve, reject) => {
      const apiUrl = `https://api.github.com/users/kodhanjo/repos?access_token=${environment.apiUrl}`;
      this.http
        .get<any>(apiUrl)
        .toPromise()
        .then(res => {
